...
const divideData = async (configData, socket, io) => {
  ...
  fs.appendFile(loggingFilePath,`[MASTER] step= ${step}` +"\n", function (err) {}); 
  let numOfrows = 0;
...
exports.configuration = (socket, io) => {
  ...
  fs.appendFile(loggingFilePath,"[MASTER] Master has been instailized socket"+"\n", function (err) {}); 
  //send meta data to clients
  socket.emit("updateMetadata", configData);
  ...
  socket.on("status", function (data) {
    ...
    fs.appendFile(loggingFilePath,`[MASTER] one of tablet servers has been connected # tabletServers = ${configData.tabletServerCounter}`+"\n", function (err) {});

    console.log(`[MASTER] System has ${configData.numOfTablets} tablets.`);
    fs.appendFile(loggingFilePath,`[MASTER] System has ${configData.numOfTablets} tablets.`+"\n", function (err) {});
  });

  socket.on("lazyUpdate", function (data) {
    console.log(`[MASTER] Server has recieved edit vector`);
    fs.appendFile(loggingFilePath,`[MASTER] Server has recieved edit vector`+"\n", function (err) {});
  ...
  socket.on("lazyCreate", function (data) {
    ...
    fs.appendFile(loggingFilePath, `[MASTER] Server has recieved created vector from tablet` +"\n", function (err) {});
  ...
  socket.on("lazyDelete", function (data) {
    ...
    fs.appendFile(loggingFilePath, `[MASTER] Server has recieved deleted vector from Tablet` +"\n", function (err) {});
  ...
  socket.on("deleteAndRebalance", async function (data) {
    ...
    fs.appendFile(loggingFilePath, "[MASTER] Recieved Data should be deleted from other Tablet" +"\n", function (err) {});
    console.log(DeletedVector);

    DeletedVector.map((vector) => {
      vector.map(async (el) => {
        console.log(`[MASTER] row :  ${el} has been deleted`);
        fs.appendFile(loggingFilePath, `[MASTER] row :  ${el} has been deleted` +"\n", function (err) {});
    ...
    fs.appendFile(loggingFilePath, `[MASTER] finish updating master` +"\n", function (err) {});
  ...
  socket.on("disconnect", function () {
    ...
    fs.appendFile(loggingFilePath, `[MASTER] tablet server ${serverID} has been disconnected` +"\n", function (err) {});
    if (configData.tabletServerCounter !== 0) {
      configData.tabletServers.splice(index, 1);
      divideData(configData, socket, io);
    } else {
      console.log(`[MASTER] no tablet server are ready`);
      fs.appendFile(loggingFilePath, `[MASTER] no tablet server are ready` +"\n", function (err) {});
      configData = {
        tabletServerCounter: 0,
        numOfTablets: 0,
        tabletServers: [],
...
const lazyDelete = async (data, socket, io) => {
  ...
  fs.appendFile(loggingFilePath, `[MASTER] Confirm lazy delete` +"\n", function (err) {});
  console.log(`[Master] recieve deleted vector from server: ${tabletServerID}`);
  fs.appendFile(loggingFilePath, `[Master] recieve deleted vector from server: ${tabletServerID}` +"\n", function (err) {});
  DeletedVector.map((vector) => {
    vector.map(async (el) => {
      console.log(`[MASTER] row :  ${el} has been deleted`);
      fs.appendFile(loggingFilePath,`[MASTER] row :  ${el} has been deleted`+"\n", function (err) {});
  ...
  fs.appendFile(loggingFilePath,`[MASTER] Send re-balance request to the server: ${otherServerID}`+"\n", function (err) {});
  io.to(socketID).emit("reBalance", {});
};
const lazyUpdate = async (data, socket, io) => {
  console.log(`[MASTER] Confirm lazy update`);
  fs.appendFile(loggingFilePath,`[MASTER] Confirm lazy update`+"\n", function (err) {});
  ...
  movies.forEach(async (movie, index) => {
    ...
    fs.appendFile(loggingFilePath,`[MASTER] update movie id: ${ids[index]}`+"\n", function (err) {});
  });
};
const lazyCreate = async (data, socket, io) => {
  ...
  fs.appendFile(loggingFilePath,`[MASTER] Confirm lazy create`+"\n", function (err) {});
